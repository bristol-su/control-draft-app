    ...
    <div>
        ...
        <b-table :fields="fields" :items="users">
            ...
            <template v-slot:cell(actions)="data">
                <a :href="'/user/' + data.item.id"><b-button size="sm" variant="outline-secondary">View</b-button></a>
                <b-button size="sm" variant="outline-danger" @click="removeUser(data.item)">Delete</b-button>
            </template>
        </b-table>

        <b-modal id="add-user">
            <user-adder :group="group" @input="addUser"></user-adder>
        </b-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$control.group().users(this.group.id)
        ...
        methods: {
            removeUser(user) {
                this.$bvModal.msgBoxConfirm('Are you sure you want to remove the user from the group?', {
                    ...
                    .then(confirmed => {
                        if(confirmed) {
                            this.$control.group().removeUser(this.group.id, user.id)
                                .then(response => {
                                    this.$notify.success('User removed!');
                                    this.users.splice(this.users.indexOf(user), 1);
                                })
                                .catch(error => this.$notify.alert('User could not be removed.'))
                        }
                    });
            },
