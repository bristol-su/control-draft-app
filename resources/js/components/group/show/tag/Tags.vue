<template>
    <div>
        ...
        <b-modal id="add-tag">
            <tag-adder :group="group" @input="addTag"></tag-adder>
        </b-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$control.group().tags(this.group.id)
        ...
        methods: {
            removeTag(tag) {
                this.$bvModal.msgBoxConfirm('Are you sure you want to remove the tag from the group?', {
                    ...
                    .then(confirmed => {
                        if(confirmed) {
                            this.$control.group().untag(this.group.id, tag.id)
                                .then(response => {
