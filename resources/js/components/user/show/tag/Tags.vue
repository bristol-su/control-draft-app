    ...
    <div>
        ...
        <b-table :fields="fields" :items="tags">
            ...
            <template v-slot:cell(actions)="data">
                <a :href="'/user-tag/' + data.item.id"><b-button size="sm" variant="outline-secondary">View</b-button></a>
                <b-button size="sm" variant="outline-danger" @click="removeTag(data.item)">Delete</b-button>
            </template>
        </b-table>

        <b-modal id="add-tag">
            <tag-adder :user="user" @input="addTag"></tag-adder>
        </b-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$control.user().tags(this.user.id)
        ...
        methods: {
            removeTag(tag) {
                this.$bvModal.msgBoxConfirm('Are you sure you want to remove the tag from the user?', {
                    ...
                    .then(confirmed => {
                        if(confirmed) {
                            this.$control.user().untag(this.user.id, tag.id)
                                .then(response => {
